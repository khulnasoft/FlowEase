...
import type { Class, DirectoryLoader, Types } from 'flowease-core';
import {
...
export class LoadNodesAndCredentials {
	...
	async init() {
		...
		if (!inE2ETests) {
			...
			this.excludeNodes.push('flowease-nodes-base.e2eTest');
		}
		...
		for (const nodeModulesDir of basePathsToScan) {
			await this.loadNodesFromNodeModules(nodeModulesDir, 'flowease-nodes-base');
