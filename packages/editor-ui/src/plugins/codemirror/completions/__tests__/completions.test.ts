...
describe('Resolution-based completions', () => {
	...
	describe('recommended completions', () => {
		...
		test('should recommend extractEmail() for: {{ "string with test@flowease.khulnasoft.com in it".| }}', () => {
			vi.spyOn(workflowHelpers, 'resolveParameter').mockReturnValueOnce(
				// @ts-expect-error Spied function is mistyped
				'string with test@flowease.khulnasoft.com in it',
			);
			const options = completions('{{ "string with test@flowease.khulnasoft.com in it".| }}');
			expect(options?.[0]).toEqual(
				expect.objectContaining({ label: 'extractEmail()', section: RECOMMENDED_SECTION }),
			);
		});

		test('should recommend extractDomain(), isEmail() for: {{ "test@flowease.khulnasoft.com".| }}', () => {
			vi.spyOn(workflowHelpers, 'resolveParameter').mockReturnValueOnce(
				// @ts-expect-error Spied function is mistyped
				'test@flowease.khulnasoft.com',
			);
			const options = completions('{{ "test@flowease.khulnasoft.com".| }}');
			expect(options?.[0]).toEqual(
		...
		test('should recommend extractDomain(), extractUrlPath() for: {{ "https://flowease.khulnasoft.com/pricing".| }}', () => {
			vi.spyOn(workflowHelpers, 'resolveParameter').mockReturnValueOnce(
				// @ts-expect-error Spied function is mistyped
				'https://flowease.khulnasoft.com/pricing',
			);
			const options = completions('{{ "https://flowease.khulnasoft.com/pricing".| }}');
			expect(options?.[0]).toEqual(
				expect.objectContaining({ label: 'extractDomain()', section: RECOMMENDED_SECTION }),
			);
			expect(options?.[1]).toEqual(
