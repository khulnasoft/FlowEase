...
export class I18nClass {
	...
	shortNodeType(longNodeType: string) {
		return longNodeType.replace('flowease-nodes-base.', '');
	}
	...
	credText() {
		...
		const credentialPrefix = `flowease-nodes-base.credentials.${credentialType}`;
		const context = this;
	...
	nodeText() {
		...
		const initialKey = `flowease-nodes-base.nodes.${nodeType}.nodeView`;
		const context = this;
		...
		return {
			...
			eventTriggerDescription(nodeType: string, eventTriggerDescription: string) {
				return context.dynamicRender({
					key: `flowease-nodes-base.nodes.${nodeType}.nodeView.eventTriggerDescription`,
...
export function addNodeTranslation(
	...
	const oldNodesBase = i18nInstance.global.messages[language]['flowease-nodes-base'] || {};
	...
	const newNodesBase = {
		'flowease-nodes-base': Object.assign(oldNodesBase, { nodes: updatedNodes }),
	};
...
export function addCredentialTranslation(
	...
	const newNodesBase = {
		'flowease-nodes-base': Object.assign(oldNodesBase, { credentials: updatedCredentials }),
	};
