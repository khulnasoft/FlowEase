...
export class FloweaseStructuredOutputParser<T extends z.ZodTypeAny> extends StructuredOutputParser<T> {
	async parse(text: string): Promise<z.infer<T>> {
		try {
			...
			// eslint-disable-next-line flowease-nodes-base/node-execute-block-wrong-error-thrown
			...
			resolve(moduleName, parentDirname) {
				if (moduleName === 'json-schema-to-zod') {
					return require.resolve(
						'@flowease/flowease-nodes-langchain/node_modules/json-schema-to-zod/dist/cjs/jsonSchemaToZod.js',
						{
							paths: [parentDirname],
						},
					);
				...
				if (moduleName === 'zod') {
					return require.resolve('@flowease/flowease-nodes-langchain/node_modules/zod.cjs', {
						paths: [parentDirname],
					});
				}
				return;
