 ...
 * eslint-plugin-flowease-local-rules and exposed as 'flowease-local-rules/<rule-name>'.
 */
module.exports = {
	/**
	 ...
	 * (abundant in the flowease codebase):
	 *
	 * Valid:
	 * ```js
	...
	'no-uncaught-json-parse': {
		meta: {
			...
			docs: {
				description:
					'Calls to `JSON.parse()` must be replaced with `jsonParse()` from `flowease-workflow` or surrounded with a try/catch block.',
				recommended: 'error',
			},
			schema: [],
			messages: {
				noUncaughtJsonParse:
					'Use `jsonParse()` from `flowease-workflow` or surround the `JSON.parse()` call with a try/catch block.',
			},
		},
		defaultOptions: [],
		create(context) {
	...
	'no-json-parse-json-stringify': {
		meta: {
			...
			docs: {
				description:
					'Calls to `JSON.parse(JSON.stringify(arg))` must be replaced with `deepCopy(arg)` from `flowease-workflow`.',
				recommended: 'error',
			},
			messages: {
