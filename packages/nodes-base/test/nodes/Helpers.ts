...
import { BinaryDataService, Credentials, constructExecutionMetaData } from 'flowease-core';
...
export function setup(testData: WorkflowTestData[] | WorkflowTestData) {
	...
	for (const nodeName of nodeNames) {
		if (!nodeName.startsWith('flowease-nodes-base.')) {
			throw new ApplicationError(`Unknown node type: ${nodeName}`, { level: 'warning' });
		}
		const loadInfo = knownNodes[nodeName.replace('flowease-nodes-base.', '')];
		if (!loadInfo) {
			throw new ApplicationError(`Unknown node type: ${nodeName}`, { level: 'warning' });
		}
		const sourcePath = loadInfo.sourcePath.replace(/^dist\//, './').replace(/\.js$/, '.ts');
