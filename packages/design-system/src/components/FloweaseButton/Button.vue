<template>
	...
	>
		<span v-if="loading || icon" :class="$style.icon">
			<FloweaseSpinner v-if="loading" :size="size" />
			<FloweaseIcon v-else-if="icon" :icon="icon" :size="size" />
		</span>
...
import FloweaseSpinner from '../FloweaseSpinner';
import type { ButtonProps } from '@/types/button';

const $style = useCssModule();
const $attrs = useAttrs();

defineOptions({ name: 'FloweaseButton' });
...
watchEffect(() => {
	if (props.element === 'a' && !props.href) {
		console.error('flowease-button:href is required for link buttons');
	}
});
...
.el-button {
	...
	&.el-color-dropdown__link-btn {
		@include flowease-button-secondary;
	}
}
</style>
...
.success {
	@include flowease-button-success;
}

.warning {
	@include flowease-button-warning;
}

.danger {
	@include flowease-button-danger;
}
