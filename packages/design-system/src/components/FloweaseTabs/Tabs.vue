<template>
	<div :class="['flowease-tabs', $style.container]">
		<div v-if="scrollPosition > 0" :class="$style.back" @click="scrollLeft">
			<FloweaseIcon icon="chevron-left" size="small" />
		</div>
		<div v-if="canScrollRight" :class="$style.next" @click="scrollRight">
			<FloweaseIcon icon="chevron-right" size="small" />
		</div>
		<div ref="tabs" :class="$style.tabs">
			...
			>
				<flowease-tooltip :disabled="!option.tooltip" placement="bottom">
					<template #content>
					...
					>
						<div>
							...
							<span :class="$style.external"
								><FloweaseIcon icon="external-link-alt" size="small"
							/></span>
						</div>
					</a>
					...
					<div
						...
						@click="() => handleTabClick(option.value)"
					>
						<FloweaseIcon v-if="option.icon" :icon="option.icon" size="medium" />
